...
export default class Poster extends Service {
  ...
  async handleSnapshot() {
    const html = this.ctx.query.html;
    const htmlRedisKey = createHash('sha256').update(html).digest('hex');
  ...
  async generateSnapshot(htmlRedisKey) {
    ...
    const path = `${this.app.config.static.dir}/upload/${date.getFullYear()}/${formatNumber(date.getMonth() + 1)}`;
    try {
